...
const scoreTokenAgainstItem = (token, fields) => {
  ...
  if (priceToken && fields.priceTokens.length) {
    for (const price of fields.priceTokens) {
      if (price === priceToken) best = Math.max(best, 70);
      else if (price.startsWith(priceToken)) best = Math.max(best, 45);
      else if (price.includes(priceToken)) best = Math.max(best, 28);
    }
  }
...
const buildFields = (item) => {
  ...
  return {
    ...
    // Item lama punya satu `price`; item dengan pilihan (Frozen/Goreng, dll)
    // punya `variants` — keduanya ikut dicocokkan saat pelanggan cari "25k".
    priceTokens: [
      item.price,
      ...(Array.isArray(item.variants) ? item.variants.map((v) => v?.price) : []),
    ]
